...
The mapping study in \Cref{ch:mapping} closed on a gap with two sides. HPC has no operational mechanism to act on environmental cost where scheduling and allocation decisions are made, and its instruments for judging whole systems measure efficiency where sufficiency is what counts. This chapter takes a first step into each. The first two sections work inside the batch scheduler and ask whether changing when jobs start can lower a center's carbon or water footprint without hurting performance. The last turns to evaluation, toward measures that weigh a computation against its environmental cost rather than its speed alone.
...
Greenfilling is not competitive in any evaluated scenario. \Cref{fig:greenfilling-deltas} shows the paired changes relative to EASY, and \Cref{tab:greenfilling-breakdown} breaks the medians down by machine, regime, and objective. No scenario median footprint change reaches 0.1\% in magnitude, for carbon or for water, while mean bounded slowdown rises in every one of the 288 pairs, between +24.5\% and +757.2\%, with scenario medians between +161.5\% and +323.6\%. The 95th-percentile waiting time of replayed jobs also rises in all 288 pairs. Even the most favorable case, Mustang under the slack regime, where carbon falls in 30 of 36 windows under carbon Greenfilling and water falls in 29 of 36 under water Greenfilling, buys those reductions at a median slowdown increase of +323.6\%. Under stress the Mustang medians turn slightly positive and Trinity stays near zero in both regimes. Zero of the 288 pairs reduce the targeted footprint without increasing mean bounded slowdown.
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{shifting_ceiling_at_24h}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{shifting_displaced_jobs}
\caption{Share of jobs the static analysis displaces as a function of $d_m$ (left) and the distribution of the displacement $t^* - r_j$ among displaced jobs at a $d_m$ of 24 hours (right).}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{shifting_bsld_distribution}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{shifting_saving_vs_bsld}
...
% TODO: decide if this paragraph is really necessary here. I don't think so.
% The next step is to implement the heuristic as a scheduler variant and run it through the same campaign infrastructure. The static ceiling bounds what those experiments can find, and the distance between the two will measure what node capacity and queue contention take away.

\section{Evaluating HPC Systems for Sufficiency}
